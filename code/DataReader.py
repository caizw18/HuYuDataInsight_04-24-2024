...
def prepare_X(raw_X):
    ...
    ### YOUR CODE HERE
    feature1 = -np.sum(np.abs(raw_image - np.flip(raw_image, axis=2)), axis=(1, 2)) / 256
    ### END YOUR CODE
    ...
    ### YOUR CODE HERE
    shape_feature3 = np.shape(feature1)
    feature3 = np.ones(shape_feature3)
    ### END YOUR CODE

    # Stack features together in the following order.
    # [Feature 3, Feature 1, Feature 2]
    ### YOUR CODE HERE
    X = np.stack([feature3, feature1, feature2], axis=1)
    ### END YOUR CODE
    return X
